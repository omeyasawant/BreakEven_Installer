...
const BUNDLED_LINUX_DEPENDENCIES = [
  {
    fileName: "libfuse2_amd64.deb",
    packageName: "libfuse2",
    platformLabel: "Ubuntu/Debian",
  },
  {
    fileName: "fuse-libs_x86_64.rpm",
    packageName: "fuse-libs",
    platformLabel: "RedHat/Fedora",
  }
];
let hasLoggedBundledLinuxDependencies = false;

function resolveMacArch() {
  const requestedArch = String(process.env.INSTALLER_MAC_ARCH || "universal")
    .trim()
    .toLowerCase();

  if (!MAC_ARCH_OVERRIDES.has(requestedArch)) {
    throw new Error(
      "Unsupported INSTALLER_MAC_ARCH '" +
        requestedArch +
        "'. Expected one of: " +
...
function verifyMacBuildIconAsset() {
  const macIconPath = path.join(installerDir, "assets", "icon.icns");
  if (!fs.existsSync(macIconPath)) {
    throw new Error(
      "Missing macOS icon asset at " +
        macIconPath +
        ". The installer app/DMG would show a generic icon.",
    );
  }
}
...
function cleanForgeOutputForTarget(platform, ext) {
  ...
  const targetMakeDir = path.join(makeDir, ext);
  if (fs.existsSync(targetMakeDir)) {
    fs.rmSync(targetMakeDir, { recursive: true, force: true });
    console.log(
      `🧹 Removed stale make output for .${ext}: ${targetMakeDir}`,
    );
  }
  ...
  for (const childName of fs.readdirSync(outDir)) {
    if (childName === "make") {
      continue;
    }

    const childPath = path.join(outDir, childName);
    fs.rmSync(childPath, { recursive: true, force: true });
    console.log(
      `🧼 Reset Linux packager staging before .${ext} build: ${childPath}`,
    );
  }
}
...
function getBundledLinuxDependencyDir() {
  return path.join(installerDir, "dependencies", "linux");
}

function getBundledLinuxDependencyEntries() {
  return BUNDLED_LINUX_DEPENDENCIES.map((dependency) => ({
    ...dependency,
    sourcePath: path.join(getBundledLinuxDependencyDir(), dependency.fileName),
  }));
}

function logBundledLinuxDependencies() {
  console.log("🔍 Checking for bundled FUSE dependencies...");
  const dependencyEntries = getBundledLinuxDependencyEntries();
  let foundCount = 0;

  for (const dependency of dependencyEntries) {
    if (fs.existsSync(dependency.sourcePath)) {
      foundCount += 1;
      console.log("  ✅ Found " + dependency.fileName);
    } else {
      console.warn(
        "  ⚠️ Missing " +
          dependency.fileName +
          " for " +
          dependency.platformLabel +
          " (" +
  ...
  if (foundCount === dependencyEntries.length) {
    console.log("✅ FUSE dependencies copied to installer_gui/dependencies/");
  } else if (foundCount > 0) {
    console.warn(
      "⚠️ Some bundled FUSE dependency packages are missing. Linux installers will fall back to package-manager dependencies or extract-and-run mode.",
    );
  } else {
    console.warn(
      "⚠️ No bundled FUSE dependency packages were found. Linux installers will rely on package-manager dependencies or extract-and-run mode.",
    );
  }
}

function preparePlatformScopedResourcesForBuild(platform) {
  ...
  if (platform !== "linux") {
    moveRequests.push(
      {
        sourcePath: path.join(installerDir, "dependencies", "linux"),
        backupPath: path.join(backupRoot, "dependencies", "linux"),
        label: "Linux dependency bundle directory",
      },
      {
        sourcePath: path.join(installerDir, "assets", "postinst"),
        backupPath: path.join(backupRoot, "assets", "postinst"),
        label: "Linux postinst script",
      },
      {
        sourcePath: path.join(
          installerDir,
          "assets",
          "breakeveninstaller.desktop",
        ),
  ...
  if (platform !== "win32") {
    moveRequests.push({
      sourcePath: path.join(installerDir, "service_host"),
      backupPath: path.join(backupRoot, "service_host"),
      label: "Windows service host sources",
    });
  }

  if (platform !== "darwin") {
    moveRequests.push({
      sourcePath: path.join(installerDir, "assets", "icon.icns"),
      backupPath: path.join(backupRoot, "assets", "icon.icns"),
      label: "macOS icon asset",
    });
  }
  ...
  for (const request of moveRequests) {
    ...
    fs.mkdirSync(path.dirname(request.backupPath), { recursive: true });
    fs.renameSync(request.sourcePath, request.backupPath);
  ...
  return () => {
    if (!movedRequests.length) {
      return;
    }

    for (const request of movedRequests.reverse()) {
      fs.mkdirSync(path.dirname(request.sourcePath), { recursive: true });
      fs.renameSync(request.backupPath, request.sourcePath);
    }

    fs.rmSync(backupRoot, { recursive: true, force: true });
    console.log(
      "♻️ Restored platform-scoped packaging resources after " +
        platform +
        " build.",
    );
  };
...
function prepareBundledLinuxDependenciesForBuild(platform) {
  if (platform === "linux") {
    if (!hasLoggedBundledLinuxDependencies) {
      logBundledLinuxDependencies();
      hasLoggedBundledLinuxDependencies = true;
    }
  ...
  const dependencyEntries = getBundledLinuxDependencyEntries();
  const backupDir = path.join(installerDir, ".ci-assets", "bundled-linux-dependencies");
  ...
  for (const dependency of dependencyEntries) {
    ...
    fs.renameSync(
      ...
      path.join(backupDir, dependency.fileName),
    );
    movedCount += 1;
  }
  ...
  if (movedCount) {
    console.log(
      "🧹 Temporarily removed " +
        movedCount +
        " bundled Linux FUSE package(s) from " +
        platform +
        " build context.",
    );
  }
  ...
  return () => {
    ...
    fs.mkdirSync(getBundledLinuxDependencyDir(), { recursive: true });

    for (const dependency of dependencyEntries) {
      const backupPath = path.join(backupDir, dependency.fileName);
      if (!fs.existsSync(backupPath)) {
        continue;
      }

      fs.renameSync(backupPath, dependency.sourcePath);
    }

    fs.rmSync(backupDir, { recursive: true, force: true });
    console.log(
      "♻️ Restored bundled Linux FUSE package(s) after " +
        platform +
        " build.",
    );
  };
...
function filterForgeResourceEntries(entries, excludedEntries) {
  ...
  return filteredEntries.length ? filteredEntries : undefined;
}

function configurePackagerResourcesForTarget(platform, ext) {
  ...
  if (!packagerConfig) {
    throw new Error("Unable to locate forge.packagerConfig in package.json for target-specific packaging.");
  }

  const excludedExtraResources = new Set();
  const excludedAsarUnpackEntries = new Set();
  const changeNotes = [];

  if (platform !== "win32") {
    excludedExtraResources.add("service_host");
    excludedAsarUnpackEntries.add("service_host/**");
    changeNotes.push("removed Windows-only service_host packaging resources");
  }

  if (platform === "win32" && ext === "exe") {
    excludedExtraResources.add("BreakEvenClient_Template.zip");
    changeNotes.push("excluded BreakEvenClient_Template.zip from Squirrel app payload");
  }

  const filteredExtraResources = filterForgeResourceEntries(
    packagerConfig.extraResource,
    excludedExtraResources,
  );
  if (filteredExtraResources) {
    packagerConfig.extraResource = filteredExtraResources;
  } else {
    delete packagerConfig.extraResource;
  }

  const filteredAsarUnpackEntries = filterForgeResourceEntries(
    packagerConfig.asarUnpack,
    excludedAsarUnpackEntries,
  );
  if (filteredAsarUnpackEntries) {
    packagerConfig.asarUnpack = filteredAsarUnpackEntries;
  } else {
    delete packagerConfig.asarUnpack;
  }
  ...
  if (platform === "win32" && ext === "exe" && Array.isArray(makers)) {
    for (const maker of makers) {
      if (maker?.name === "@electron-forge/maker-squirrel" && maker.config) {
        delete maker.config.processStart;
      }
    }
    changeNotes.push("skipped post-install auto-launch");
  }

  fs.writeFileSync(packageJsonPath, JSON.stringify(packageJson, null, 2) + os.EOL);
  if (changeNotes.length) {
    console.log(
      "📦 Applied target packaging overrides for ." +
        ext +
        ": " +
        changeNotes.join("; ") +
        ".",
...
async function runBuild() {
  ...
  for (const { ext, maker } of distributables) {
    ...
    const restorePackageJson = configurePackagerResourcesForTarget(platform, ext);
    cleanPackagerTempDir();
    const isolatedTempDir = createIsolatedTempDir(platform, ext);
    const restorePlatformScopedResources =
      preparePlatformScopedResourcesForBuild(platform);
    const restoreBundledLinuxDependencies =
      prepareBundledLinuxDependenciesForBuild(platform);
    cleanForgeOutputForTarget(platform, ext);

    let forgeCmd = `npx electron-forge make --platform ${platform}`;
    if (platform === "linux") forgeCmd += " --arch x64";
    if (platform === "darwin") forgeCmd += ` --arch ${resolveMacArch()}`;
    ...
    try {
      ...
      restorePackageJson();
      restoreBundledLinuxDependencies();
      restorePlatformScopedResources();
    }
