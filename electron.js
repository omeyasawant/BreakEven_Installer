...
app.whenReady().then(async () => {
  ...
  if (process.platform === "win32" && !wasLaunchedFromElevatedBootstrapper()) {
...
function makeAutoStartScript(scriptPath, name, targetDir) {
  ...
  if (platform === "win32") {
    ...
    const launchDir = targetDir || path.dirname(scriptPath);
    const lowerPath = String(scriptPath).toLowerCase();
    const launchCommand = lowerPath.endsWith(".py")
    ...
    const lowerPath = String(scriptPath).toLowerCase();
    const launchDir = targetDir || path.dirname(scriptPath);
    const execParts = lowerPath.endsWith(".py")
      ? ["python3", scriptPath]
      : lowerPath.endsWith(".sh")
        ? ["/bin/bash", scriptPath]
        : [scriptPath];
    const execLine = execParts.map((part) => quotePosixExecArg(part)).join(" ");
  ...
  try {
    ...
    if (process.platform === "linux") {
      ...
      if (fs.existsSync(debPath)) {
        ...
        try {
          await execCommandLogged("pkexec", ["dpkg", "-i", debPath], event);
      ...
      if (fs.existsSync(rpmPath)) {
        ...
        try {
          await execCommandLogged("pkexec", ["rpm", "-i", rpmPath], event);
          const launchPath = await waitForLinuxDashboardLaunchPath(
            event,
            dashboardDir,
          );
...
async function checkAndInstallFUSE(event) {
  ...
  const fuseCheck = await execCommandLogged("dpkg", ["-s", "libfuse2"], event, {
    ignoreFailure: true,
  });
  if (fuseCheck.exitCode === 0) {
    sendLog(
      event,
      "✅ FUSE is installed - AppImages will run in native mode (best performance)",
    );
    return { success: true, alreadyInstalled: true };
  }
  ...
  sendLog(
    event,
    "ℹ️ FUSE not installed, attempting to install from Ubuntu repositories...",
  );

  const aptInstallResult = await execCommandLogged(
    "pkexec",
  ...
  if (aptInstallResult.exitCode === 0) {
    sendLog(event, "✅ FUSE installed successfully from Ubuntu repositories");
    return { success: true, installed: true, source: "apt" };
  }

  sendLog(
    event,
    "ℹ️ Repository install did not complete, attempting bundled FUSE package...",
  );

  // FUSE not installed, try to install from bundled package
  sendLog(
    event,
    "ℹ️ FUSE not installed, attempting to install from bundled package...",
  );

  // Try to install from bundled .deb package
  ...
  if (debPath && fs.existsSync(debPath)) {
    try {
      ...
      const installResult = await execCommandLogged(
        "pkexec",
        ["dpkg", "-i", debPath],
        event,
        {
      ...
      sendLog(
        event,
        "⚠️ Could not install bundled FUSE package (no elevated permissions)",
      );
    }
  } else {
    sendLog(event, "ℹ️ No bundled FUSE package found");
  }

  // FUSE installation failed or not available
  sendLog(
    event,
  ...
  sendLog(
    event,
    "💡 Optional: Install FUSE for better performance: sudo apt install libfuse2",
  );
  return {
...
ipcMain.handle("start-installation", async (event, installData) => {
  ...
  try {
    ...
    if (skipPythonSetup) {
      sendLog(
        event,
        "⏭️ Skipping Python raw script setup and Python requirements (temporarily disabled)",
      );
    } else {
