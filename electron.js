...
async function LaunchService(event, label, script, destinationPath) {
    return new Promise((resolve, reject) => {
        ...
        const readyFile = path.join(destinationPath, `${label.toLowerCase()}_ready.txt`);
        try { if (fs.existsSync(readyFile)) fs.unlinkSync(readyFile); } catch (_) { }
        ...
        */
        ...
        const intervalId = setInterval(() => {
            ...
            if (fs.existsSync(readyFile)) {
                clearInterval(intervalId);
                sendLog(event, `✅ ${label} signaled ready via ${path.basename(readyFile)}`);
            ...
            if (elapsed >= timeoutMs) {
                ...
                if (!exited) {
                    ...
                    sendLog(event, `⚠️ ${label} did not signal ready but is still running; assuming OK`);
...
ipcMain.handle('start-installation', async (event, installData) => {
    ...
    try {
        ...
        */
        ...
        await new Promise((resolve, reject) => {
            execFile(condaPath, ['create', '-y', '-n', envName, 'python=3.10'], (err) => {
